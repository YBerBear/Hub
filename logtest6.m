...
mustexit = 0;    # the main loop (initiated by the next line "do") below runs until this changes to 1;
do                                                    
	...
	%% (1)
	output_file_base = "c:\\eugene\\testsofgnuplotandoctave\\hub_reports\\report";
	...
	## check if the condition to terminate the loop is met:
	## there must be a file or folder in the directory "stopit"
	try
		signaldir = dir ("C:\\stopit\\*");
		nameoffilepresent = signaldir(1).name;
		mustexit = 1;
	catch
		mustexit = 0;
	end_try_catch
