...
{
	WSADATA wsaData;
	int iRes, iS, iError,iStrToSendLen, iBytesReceived,iTotalBytesReceived,iCharCount;
	...
	FILE * thesource;
	size_t size_read;
	char  buf_read[512];  // previously: 64
	char current_char;
	...
	do
	{
		while (iCharCount < 512) 
		{
			...
			{
				//buf_read[iCharCount]='&';
				                //sprintf(&buf_read[iCharCount],"%d",129);
				break;
			}
			else
			{
